...
/**
  ...
  */
class AkkaHttpTransport(actorSystem: ActorSystem, apiHost: String)
    extends HttpAsyncTransport {
  ...
  private val connectionFlow: Flow[HttpRequest,
                                   HttpResponse,
  ...
  /**
    ...
    */
  private def processHttpResponse(
      response: HttpResponse): Future[WeatherError \/ JValue] = {
  ...
  private def getResponseContent(
      response: HttpResponse): WeatherError \/ Future[String] =
      ...
      case _ =>
        try {
          ...
          case _: java.util.concurrent.TimeoutException =>
            \/.left(
              TimeoutError(
                s"HTTP entity didn't processed in ${timeout.toString()}"))
          case NonFatal(e) =>
  ...
  private def parseJson(content: String): WeatherError \/ JValue =
    parseOpt(content) match {
      ...
      case None =>
        \/.left(
          ParseError(
            s"OpenWeatherMap Error: string [$content] doesn't contain JSON"))
    }
}
