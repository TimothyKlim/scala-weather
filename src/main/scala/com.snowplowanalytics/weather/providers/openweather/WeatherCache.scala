...
trait WeatherCache[W <: OwmResponse] {
  ...
  protected val cache: Cache =
    new SynchronizedLruMap[CacheKey, WeatherError \/ W](cacheSize)

  if (geoPrecision < 1)
    throw new IllegalArgumentException(
      "OwmCacheClient geoPrecision must be greater than zero")
  ...
  def eventToCacheKey(timestamp: Timestamp, position: Position): CacheKey = {
    ...
    val roundPosition = Position(roundCoordinate(position.latitude),
