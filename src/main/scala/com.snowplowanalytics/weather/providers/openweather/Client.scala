...
trait Client[Response[_]] {
  ...
  def historyById(
    ...
    val request = OwmHistoryRequest(
      "city",
      Map("id" -> id.toString) ++ (("start", start)) ++ (("end", end)) ++ (("cnt",
                                                                            cnt)) ++ (("type",
                                                                                       measure.map(
                                                                                         _.toString))))
    receive(request)
  ...
  def historyByName(
    ...
    val request = OwmHistoryRequest(
      "city",
      Map("q" -> query) ++ (("start", start)) ++ (("end", end)) ++ (("cnt",
                                                                     cnt)) ++ (("type",
                                                                                measure.map(
                                                                                  _.toString))))
    receive(request)
  ...
  def forecastById(id: Int, cnt: OptArg[Int] = None): Response[Forecast] =
    receive(
      OwmForecastRequest("city",
                         Map("id" -> id.toString, "cnt" -> cnt.toString)))
  ...
  def forecastByName(name: String,
                     ...
                     cnt: OptArg[Int] = None): Response[Forecast] = {
    val query = name + country.map("," + _).getOrElse("")
    receive(
      OwmForecastRequest("city", Map("q" -> query, "cnt" -> cnt.toString)))
  }
  ...
  /**
    ...
    * @return either error or forecast wrapped in `Response`
    */
  def forecastByCoords(lat: Float,
                       lon: Float,
                       cnt: OptArg[Int] = None): Response[Weather] =
    receive(
      OwmForecastRequest("weather",
  ...
  def currentByName(name: String,
                    country: OptArg[String],
                    cnt: OptArg[Int] = None): Response[Current] = {
    val query = name + country.map("," + _).getOrElse("")
    receive(
      OwmCurrentRequest("weather", Map("q" -> query, "cnt" -> cnt.toString)))
  }
  ...
  /**
    ...
    */
  protected[openweather] def extractWeather[W: Manifest](
      response: JValue): WeatherError \/ W =
    response.extractOpt[W] match {
      ...
      case None =>
        response.extractOpt[ErrorResponse] match {
          ...
          case None =>
            \/.left(ParseError(
              s"Could not extract ${manifest[W]} from ${compactJson(response)}"))
        }
    }
}
