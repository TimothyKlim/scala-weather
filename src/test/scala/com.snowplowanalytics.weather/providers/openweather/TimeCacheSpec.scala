...
class TimeCacheSpec(implicit val ec: ExecutionEnv)
  ...
  private val newDayInKranoyarsk =
    DateTime.parse("2015-12-14T00:12:44.000+07:00")

  def e1 = rounderChecker1.getStartOfDay(nov19time) must beEqualTo(nov19begin)
  def e2 =
    rounderChecker1
      .eventToCacheKey(1445591292, Position(42.832f, 32.1101f)) must beEqualTo(
      CacheKey(1445558400, Position(43.0f, 32.0f)))

  def e3 = {
    val calendar = Calendar.getInstance(TimeZone.getTimeZone("GMT"))
    calendar.setTime(
      new Date(rounderChecker1.getStartOfDay(1445151725).toLong * 1000))
  ...
  def e4 = {
    ...
    val cacheKey =
      rounderChecker1.eventToCacheKey(1445151725, Position(0.0f, 0.0f))
    calendarStart.setTime(new Date(cacheKey.day.toLong * 1000))
    calendarEnd.setTime(new Date(cacheKey.endOfDay.toLong * 1000))
    val delta = calendarEnd.get(Calendar.DAY_OF_MONTH) - calendarStart.get(
